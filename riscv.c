...
void write_memory_2(uint32_t *memory, uint32_t address, uint16_t value) {
    ...
    if ((address & 1) == 0) { // Aligned
        size_t index = address / 4;
        int offset = address % 4;
        memory[index] = (memory[index] & ~(0xFFFFu << (offset * 8))) | ((uint32_t)value << (offset * 8));
    } else { // Unaligned
...
int main(int argc, char *argv[]) {
    ...
    //test unaligned read beginning at word boundary
    test_addr = 0x1000;
    write_memory_4(memory, test_addr, 0x12345678);
    printf("Wrote 0x12345678 at 0x%08x, read back: 0x%08x\n", test_addr, read_memory_4(memory, test_addr));
    
    uint32_t value = read_memory_4(memory, test_addr+1);
    printf("Unaligned Read back: 0x%08x\n", value);
