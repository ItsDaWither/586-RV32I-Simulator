...
// Read 2 bytes (halfword) from memory.
// The address must be even.
uint16_t read_memory_2(uint32_t *memory, uint32_t address) {
    if (address % 2 != 0) {
        fprintf(stderr, "Error: read_memory_2: address 0x%x not halfword aligned\n", address);
        exit(1);
    }
...
// Read 4 bytes (word) from memory.
// The address must be word aligned.
uint32_t read_memory_4(uint32_t *memory, uint32_t address) {
    if (address % 4 != 0) {
        fprintf(stderr, "Error: read_memory_4: address 0x%x not word aligned\n", address);
        exit(1);
    }
    if (address >= MEMORY_SIZE_BYTES) {
...
void write_memory_1(uint32_t *memory, uint32_t address, uint8_t value) {
    ...
    size_t index = address / 4;
    int offset = address % 4;
    memory[index] = (memory[index] & ~(0xFFu << (offset * 8))) | ((uint32_t)value << (offset * 8));
}

// Write 2 bytes (halfword) to memory.
// The address must be even.
void write_memory_2(uint32_t *memory, uint32_t address, uint16_t value) {
    if (address % 2 != 0) {
        fprintf(stderr, "Error: write_memory_2: address 0x%x not halfword aligned\n", address);
        exit(1);
    }
...
// Write 4 bytes (word) to memory.
// The address must be word aligned.
void write_memory_4(uint32_t *memory, uint32_t address, uint32_t value) {
    if (address % 4 != 0) {
        fprintf(stderr, "Error: write_memory_4: address 0x%x not word aligned\n", address);
        exit(1);
    }
    if (address >= MEMORY_SIZE_BYTES) {
...
int main(int argc, char *argv[]) {
    ...
    uint32_t stack_address = MEMORY_SIZE_BYTES;  // Default stack pointer at end of memory.
